...
# (1) Build a dict mapping ingredient names → list of allowed units
INGREDIENTS_UNITS = {}
csv_path = os.path.join(basedir, 'data', 'ingredients_units.csv')
with open(csv_path, newline='', encoding='utf-8') as f:
    reader = csv.DictReader(f)
    for row in reader:
        # CSV columns: "ingredient","units" (e.g. "flour","grams, cups, ounces")
        INGREDIENTS_UNITS[row['ingredient']] = [u.strip() for u in row['units'].split(',')]
...
def upload():
    ...
    # clear any previous URL/metadata
    session.pop("prefill_url", None)
    ...
    # When rendering the form, pass in both lists
    return render_template(
        "upload_recipe.html",
        message          = message,
